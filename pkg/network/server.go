			...
			if s.chain.HeaderHeight() < p1 {
				...
				if h < p1 {
					...
					err = sr.Traverse(sr.CurrentLocalStateRoot(), func(n mpt.Node, _ []byte) bool {
						...
						delete(unknownNodes, n.Hash())
						childrenPaths := mpt.GetChildrenPaths(path, n)
					...
					s.mptPool.Update(nil, unknownNodes)
...
func (s *Server) handleMPTDataCmd(p Peer, data *payload.MPTData) error {
	...
	toBeRequested := make(map[util.Uint256][]byte)
	for _, nBytes := range data.Nodes {
		...
		err := s.chain.GetStateModule().RestoreMPTNode(itm, n.Node)
		if err != nil {
			return fmt.Errorf("failed to add MPT node with hash %s and path %s: %w", n.Hash().StringBE(), hex.EncodeToString(itm), err)
		}
		childrenPaths := mpt.GetChildrenPaths(itm, n.Node)
		for h, path := range childrenPaths {
			toBeRequested[h] = path
		}

		delete(toBeRequested, n.Hash())
		s.mptPool.Update(map[util.Uint256]bool{n.Hash(): true}, childrenPaths)
	}
...
func (s *Server) requestMPTNodes(p Peer, itms map[util.Uint256][]byte) error {
	stateSyncP := s.p.Load()
...
func (s *Server) handleHeadersCmd(p Peer, h *payload.Headers) error {
	...
	if pSync := s.p.Load(); pSync != 0 && s.chain.GetStateModule().CurrentLocalHeight() != pSync && s.chain.HeaderHeight() > pSync {
		...
		err = s.requestMPTNodes(p, map[util.Uint256][]byte{stateRoot: {}})
		if err != nil {
...
func (s *Server) initStaleMemPools() {
	...
	if cfg.P2PStateExchangeExtensions {
		s.mptPool.SetResendThreshold(s.ServerConfig.MPTPoolResendThreshold, func(items map[util.Uint256][]byte) {
