...
func TestQueryTx(t *testing.T) {
	...
	e.checkNextLine(t, `Persisted:\s+false`)
	e.checkEOF(t)
	...
	e.Run(t, append(args, txHash.StringLE())...)
	e.checkNextLine(t, `Hash:\s+`+txHash.StringLE())
	e.checkNextLine(t, `Persisted:\s+true`)
	e.checkEOF(t)
...
func (e *executor) compareQueryTxVerbose(t *testing.T, tx *transaction.Transaction) {
	...
	e.checkNextLine(t, `Nonce:\s+`+strconv.FormatUint(uint64(tx.Nonce), 16)+"$")
	...
	e.checkNextLine(t, `VMState:\s+`+res[0].Execution.VMState.String()+`$`)
